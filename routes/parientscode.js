...
// //ผลเลือด ของพี่ ส่งแบบ multiple
// router.post(
//   "/uploadBloodResult2",
//   imgUpload.array("images"), // ใช้ .array() เพื่อรองรับการอัพโหลดหลายไฟล์
//   async function (req, res, next) {
//     const files = req.files; // Array ของไฟล์ที่อัพโหลด
//     const { IDcard } = req.body;
//     let date = moment(Date.now()).format();

//     // ตรวจสอบว่า files ถูกส่งมา
//     if (!files || files.length === 0) { // ตรวจสอบว่าไฟล์ถูกส่งมาหรือไม่
//       return res.status(400).send("No files uploaded");
//     }

//     // ตรวจสอบว่า IDcard ถูกส่งมา
//     if (!IDcard) { // ตรวจสอบว่า IDcard ถูกส่งมาหรือไม่
//       return res.status(400).send("IDcard is required");
//     }
    
//     // ดึงข้อมูล treatmentId ล่าสุด
//     const [row] = await pool.query(
//       `SELECT MAX(treatmentId) AS treatmentId FROM treatment JOIN patient ON treatment.HN = patient.HN WHERE treatment.IDcard = ?`,
//       [IDcard]
//     );
//     const treatmentId = row[0].treatmentId;

//     // ดึงข้อมูล brId ล่าสุด
//     const [row_1] = await pool.query(
//       `SELECT MAX(brId) AS brId FROM bloodresult WHERE treatmentId = ?`,
//       [treatmentId]
//     );
//     const lastBrId = row_1[0].brId;
    
//     // ตรวจสอบสถานะของผลเลือดล่าสุด
...
//       [treatmentId]
//     );
//     const doctorId = doctorRow[0].doctorId;

//     const conn = await pool.getConnection();
//     await conn.beginTransaction();

//     try {
//       if (statusRow.length > 0 && statusRow[0].status === "อนุมัติรับยา") {
//         return res.status(403).send("Cannot upload results because the current status is 'Approved for medication'");
//       }

//       // ถ้ามีไฟล์มากกว่าหนึ่งไฟล์
//       if (files.length > 1) { // ตรวจสอบจำนวนไฟล์
//         // อัพเดตไฟล์แรก
//         const firstFile = files[0];
//         await conn.query(
//           `UPDATE bloodresult SET picture = ?, date = ?, status = 'รออนุมัติผลเลือด' WHERE treatmentId = ?`,
//           ['images/' + firstFile.filename, date, treatmentId]
//         );
...
//           await conn.query(
//             `INSERT INTO bloodresult (picture, status, doctorId, treatmentId, date) VALUES (?, 'รออนุมัติผลเลือด', ?, ?, ?, ?)`,
//             ['images/' + file.filename, doctorId, treatmentId, date]
//           );
//         }
//       } else { // ถ้ามีไฟล์เพียงไฟล์เดียว
//         const singleFile = files[0];
//         await conn.query(
//           `UPDATE bloodresult SET picture = ?, date = ?, status = 'รออนุมัติผลเลือด' WHERE treatmentId = ?`,
//           ['images/' + singleFile.filename, date, treatmentId]
//         );
//       }
